/**
 * Custom editor for dropdown with image
 * 
 * @version 1.0.0
 ...
 */
jspreadsheet.editors.dropdownimage = function() {
    var methods = {};
    ...
    methods.updateCell = function(cell, value, x, y, instance, options) {
        if(cell) {
            if(value != undefined) {
                if(!options.style) {
                    options.style = "width:auto;height:24px;border-radius:16px";
                }
                ...
                if(items.length > 0) {
                    ...
                    for(var ite_item=0; ite_item<items.length; ite_item++) {
                        ...
                        if(item.image) {
                            images += "<img src='"+item.image+"' style='"+options.style+"'>";
                        }
                    }
